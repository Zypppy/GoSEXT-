...
local Config = MenuElement({type = MENU, name = "Veigar", id = "Veigar", leftIcon = "http://static.lolskill.net/img/champions/64/veigar.png"})
Config:MenuElement({type = MENU, name = "Auto Last Hit Settings", id = "LastHit"})
Config.LastHit:MenuElement({type = MENU, name = "Baleful Strike (Q)", id = "Q", leftIcon = "http://static.lolskill.net/img/abilities/64/VeigarBalefulStrike.png"})
        ...
        if Config.LastHit.Q.Enabled:Value() then
                for _, Minion in pairs(GetMinions(200)) do
                        if getdmg("Q", Minion, myHero) > Minion.health then
                                if CanUseSpell(myHero, _Q) and IsValidTarget(Minion, GetRange(_Q), true, myHero.pos) then
