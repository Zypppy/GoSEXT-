...
local Config = MenuElement({type = MENU, name = "Renekton", id = "Renekton", leftIcon = "http://static.lolskill.net/img/champions/64/renekton.png"})

Config:MenuElement({type = MENU, name = "Combo Settings", id = "Combo"})
Config.Combo:MenuElement({type = MENU, name = "Cull The Meek (Q)", id = "Q", leftIcon= "http://static.lolskill.net/img/abilities/64/Renekton_Cleave.png"})
Config.Combo.Q:MenuElement({name = "Enabled", id = "Enabled", value = true})
Config.Combo:MenuElement({type = MENU, name = "Ruthless Predator (W)", id = "W", leftIcon= "http://static.lolskill.net/img/abilities/64/Renekton_Execute.png"})
Config.Combo.W:MenuElement({name = "Enabled", id = "Enabled", value = true})
Config.Combo:MenuElement({type = MENU, name = "Slice And Dice (E)", id = "E", leftIcon= "http://static.lolskill.net/img/abilities/64/Renekton_SliceAndDice.png"})
Config.Combo.E:MenuElement({name = "Enabled", id = "Enabled", value = true})

Config:MenuElement({type = MENU, name = "Lane/Jungle Clear", id = "Clear"})
Config.Clear:MenuElement({type = MENU, name = "Cull The Meek (Q)", id = "Q", leftIcon= "http://static.lolskill.net/img/abilities/64/Renekton_Cleave.png"})
Config.Clear.Q:MenuElement({name = "Enabled", id = "Enabled", value = true})
Config.Clear:MenuElement({type = MENU, name = "Ruthless Predator (W)", id = "W", leftIcon= "http://static.lolskill.net/img/abilities/64/Renekton_Execute.png"})
Config.Clear.W:MenuElement({name = "Enabled", id = "Enabled", value = true})

Config:MenuElement({type = MENU, name = "Misc Settings", id = "Misc"})
Config.Misc:MenuElement({type = MENU, name = "Dont Use Slice and Dice (E) Under Tower", id = "E", leftIcon= "http://static.lolskill.net/img/abilities/64/Renekton_SliceAndDice.png"})
Config.Misc.E:MenuElement({name = "Enabled", id = "Enabled", value = true})
--Config.Misc:MenuElement({type = MENU, name = "Dominus (R) When Low HP", id = "R", leftIcon= "http://static.lolskill.net/img/abilities/64/Renekton_Dominus.png"})
--Config.Misc.R:MenuElement({name = "Enabled", id = "Enabled", value = true})
...
PrintChat("I HIGHLY SUGGEST TURNING E OFF IN COMBO , USE MANUALLY")
...
function Combo(target)
  if Config.Key.Combo:Value() then
    if Config.Combo.Q.Enabled:Value() then
      if CanUseSpell(myHero, _Q) and IsValidTarget(target, 300, false, myHero.pos) then
        Control.CastSpell(HK_Q)
      end
    end
    if Config.Combo.W.Enabled:Value() then
      if CanUseSpell(myHero, _W) and IsValidTarget(target, 200, false, myHero.pos) then
    ...
    if Config.Combo.E.Enabled:Value() then
      if CanUseSpell(myHero, _E) and IsValidTarget(target, GetRange(_E), false, myHero.pos) then
        if Config.Misc.E.Enabled:Value() then
...
function JungleClear()
  if Config.Key.LaneClear:Value() then    
    for _, Minion in pairs(GetMinions(300)) do
      if Config.Clear.Q.Enabled:Value() and CanUseSpell(myHero, _Q) and IsValidTarget(Minion, 300, false, myHero.pos) then
        Control.CastSpell(HK_Q)
      end
      if Config.Clear.W.Enabled:Value() and CanUseSpell(myHero, _W) and IsValidTarget(Minion, 300, false, myHero.pos) then
        Control.CastSpell(HK_W)
      end
    end
  end
...
function LaneClear()
  if Config.Key.LaneClear:Value() then    
    for _, Minion in pairs(GetMinions(200)) do
      ...
      if Config.Clear.W.Enabled:Value() and CanUseSpell(myHero, _W) and IsValidTarget(Minion, 200, false, myHero.pos) then
        Control.CastSpell(HK_W)
      end
    end
  end
